...
export default makeScene2D(function* (view) {
  ...
  view.add(
    <>
      ...
      <Rect
        ...
        y={-80}
      >
        <Txt {...heading2Style} textAlign='center' y={-40} fill={'#4a8e6a'} text={'FileSystemAccess'} />        
      </Rect>
      ...
      <Rect
        ...
        y={-80}
      >
        <Txt {...heading2Style} textAlign='center' y={-40} fill={'#8e8372'} text={'BufferedReader'} />        
      </Rect>
      <Line
        ref={clientArrow}
        {...arrowStyle}
        ...
        points={[
          Vector2.zero,
          () => Vector2.right.scale(clientArrowSignal() * 250),
        ]}
        x={580}
        y={-70}
      ...
      <Rect
        ...
        y={180}
      >
        <Txt {...heading2Style} textAlign='center' y={-60} fill={'#4771a1'} text={'Scheduler'} />
        <Rect {...scheduleThreadStyle} x={-110} y={30} ref={makeRef(scheduleThreads, 0)}>
          <Circle size={30} fill={'#35475a'} x={43} y={23}>
            <Txt {...heading3Style} text={'1'} x={1} y={2} />
          </Circle>
        </Rect>
        <Rect {...scheduleThreadStyle} x={0} y={30} ref={makeRef(scheduleThreads, 1)}>
          <Circle size={30} fill={'#35475a'} x={43} y={23}>
            <Txt {...heading3Style} text={'2'} x={1} y={2} />
          </Circle>
        </Rect>
        <Rect {...scheduleThreadStyle} x={110} y={30} ref={makeRef(scheduleThreads, 2)}>
          <Circle size={30} fill={'#35475a'} x={43} y={23}>
            <Txt {...heading3Style} text={'3'} x={1} y={2} />
          </Circle>
        </Rect>       
  ...
  yield* all(
    ...
    request().absolutePosition(service().absolutePosition().addX(-100).addY(50), longTransition)
  );

  // bounded elastic thread takes over
  scheduleThreads[0].save();
  yield* all(
    scheduler().opacity(1, longTransition),
    client().opacity(1, longTransition)
  );
  yield* waitFor(longTransition);
  
  yield* scheduleThreads[0].absolutePosition(service().absolutePosition().addX(100).addY(50), longTransition);
  yield* request().absolutePosition(scheduleThreads[0].absolutePosition, shortTransition);
  yield eventLoops[0].restore(shortTransition);

  // move to client
  yield* all(
    scheduleThreads[0].absolutePosition(client().absolutePosition().addY(50), longTransition),
  ...
  yield all(
    ...
    scheduler().opacity(1, shortTransition)
  );
  ...
  // back to service
  yield* all(
    scheduleThreads[0].absolutePosition(service().absolutePosition().addY(50), longTransition),
  ...
  yield* all(
    ...
    scheduleThreads[0].absolutePosition(request1Position, longTransition)
  );
  yield* all(
    scheduleThreads[0].restore(shortTransition),
